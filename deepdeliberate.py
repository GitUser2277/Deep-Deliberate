...
This script provides the command-line interface for testing AI agents
...
def display_model_configuration(enhanced_eval: bool = False):
    ...
    click.echo("\n📋 Model Configuration:")
    click.echo("   • Query Generation: DeepSeek R1 (for diverse query generation)")
    click.echo("   • Agent Execution: Azure OpenAI o4-mini (your configured agent model)")
    if enhanced_eval:
        click.echo("   • Response Evaluation: DeepSeek R1 Enhanced (5-dimension comprehensive analysis)")
    else:
        click.echo("   • Response Evaluation: DeepSeek R1 Standard (4-dimension evaluation)")
    ...
    try:
        ...
        if rate_limit_int > 20:
            click.echo("   ⚠️  Tip: If you hit rate limits, try reducing API_RATE_LIMIT_PER_MINUTE to 10-15")
...
@click.option(
    ...
    default="config.json",
    type=click.Path(path_type=Path),
    help="Configuration file path (default: config.json, or use setup/configs/customer_service_config.json)"
)
@click.option(
    "-enhanced", "--enhanced",
    is_flag=True,
...
def main(
    ...
    Examples:
        python deepdeliberate.py -file setup/agents/customer_service_agent.py -mode auto -count 25
        python deepdeliberate.py -file setup/agents/customer_service_agent.py -mode approve -persona angry_customer -config setup/configs/customer_service_config.json
        python deepdeliberate.py -file setup/agents/education_agent.py -mode auto -count 10 --enhanced -config setup/configs/education_config.json
    """
...
async def run_framework_async(cli_config: CLIConfig, enhanced: bool):
    ...
    try:
        ...
        if not config_path.exists():
            ...
            raise FrameworkError(f"Configuration file not found: {config_path}")
        ...
        successful = [i for i in result.completed_interactions if not i.agent_response.startswith("ERROR")] if result.completed_interactions else []
        avg_score = sum(i.evaluation_score for i in successful) / len(successful) if successful else None
